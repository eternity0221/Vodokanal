<form method="POST" novalidate>
    ...
    <div class="bg-white relative p-8 rounded-lg shadow-lg max-w-sm w-full">
        ...
        <h2 class="text-2xl font-semibold text-center mb-4">Создайте новый аккаунт</h2>
        <p class="text-gray-600 text-center mb-6">Введите данные для регистрации</p>

        <div class="flex flex-col gap-4">
            <div>
                <label for="name" class="block text-gray-700 text-sm font-semibold mb-2">ФИО</label>
                <input type="text" name="name" id="name" class="form-input w-full px-4 py-2 border rounded-lg text-gray-700 focus:ring-blue-500" placeholder="Иванов Иван Иваныч">
            ...
            <div>
                ...
                @error('password')
                <p class="text-red-700">{{ $message }}</p>@enderror<p class="text-gray-600 text-xs mt-1">Пароль должен быть минимум 8 символов</p>
            </div>
            <div>
                <label for="password" class="block text-gray-700 text-sm font-semibold mb-2">Повторите пароль</label>
                <input type="password" name="password_confirmation" id="password_confirmation" class="form-input w-full px-4 py-2 border rounded-lg text-gray-700 focus:ring-blue-500" placeholder="*********">
                @error('password_confirmation')<p class="text-red-700">{{ $message }}</p>@enderror
            </div>
        </div>

        <button type="submit" class="w-full bg-blue-500 mt-4 text-white px-4 py-2 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">Зарегистрироваться</button>
